...
#						 + self.random_name + " " + str(self.servers_num) + " " + self.step_size, True)
			call(["java", "-d64", "-server", "-Xms10g", "-Xmx10g", "-jar", "client.jar", self.exp, self.codec, self.k, self.r, self.z, self.random_name, str(self.servers_num), self.step_size])
...
#			 csvwriter.writerow(csvrow)
	def parse_network_logs(self, name, extension):
		...
		csvwriter = csv.writer(output)
		csvwriter.writerow(['exp', 'codec', 'random', 'k', 'r', 'z', 'servers_num', 'time(ms)', 'tag', 'size', 'server_id', 'log_type', 'machine'])
...
#			 print file
	...
	def parse_codec_logs(self):
		...
		csvwriter = csv.writer(output)
		csvwriter.writerow(['exp', 'codec', 'random', 'k', 'r', 'z', 'servers_num', 'log_type', 'time(ms)', 'tag', 'size'])
